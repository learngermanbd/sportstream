 ...
 * ```
 * sportstream://main
 * sportstream://player/{channelId}
 * sportstream://category/{categoryId}
 * sportstream://notice/{noticeId}
 * ```
 *
 * ## Usage
 * ```kotlin
...
object DeepLinkValidator {
    ...
    private const val APP_SCHEME = "sportstream"

    /** Allowed host values (null host means scheme-only URI like `sportstream://main`). */
    ...
    fun validate(uri: Uri?, caller: String = "unknown"): DeepLinkResult {
        ...
        if (scheme == APP_SCHEME) {
            // In `sportstream://main`, "main" is the host
            val host = uri.host?.lowercase()
